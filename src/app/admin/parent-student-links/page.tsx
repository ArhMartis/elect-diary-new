...
export default async function ParentStudentLinksPage() {
  ...
  const studentsByGroup = allGroups.map(group => ({
    ...group,
    students: students.filter(s => s.groupId === group.id),
  }));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-emerald-100">
        ...
        <form action={linkParentToStudent} className="space-y-4">
          {/* Выбор родителя */}
          ...
          <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
            ...
            <select
              ...
              onChange={(e) => {
                // Этот обработчик будет работать на клиенте
          ...
          <div className="bg-purple-50 rounded-xl p-4 border border-purple-200">
            ...
            >
              ...
              {students.map((s) => {
                const studentGroup = allGroups.find(g => g.id === s.groupId);
                return (
                  <option 
                    key={s.id} 
                    value={s.id}
                    ...
                    style={{ display: 'none' }}
                  >
                    {s.fullName} {studentGroup ? `(${studentGroup.name})` : "(без класса)"}
                  </option>
                );
              })}
            </select>
