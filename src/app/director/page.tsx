  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        {msg && <div className={`mb-4 p-3 rounded-xl text-sm font-medium ${msg === 'Сохранено' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>{msg}</div>}

        {loading ? (
          <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-4 border-slate-300 border-t-slate-600"></div></div>
        ) : profile ? (
            ...
            <div className="p-6 md:p-8 space-y-4">
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="text-lg font-bold text-gray-900">Режим работы</p>
                </div>
                <div className="space-y-1.5 ml-15">
                  {(editing ? DAY_KEYS : DAY_KEYS.filter(d => profile[d] && profile[d] !== "выходной")).map((key) => (
                    <div key={key} className="flex items-center gap-3 text-sm">
                      <span className="w-28 font-semibold text-gray-700 shrink-0">{DAY_LABELS[key]}</span>
                      {editing ? (
                        <input value={form[key] || ""} onChange={e => setForm({...form, [key]: e.target.value})} className="flex-1 px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-indigo-500" />
                      ) : (
                        <span className="text-gray-900">{profile[key]}</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-500 font-medium">Приём граждан</p>
                  {editing ? (
                    <input value={form.receptionHours || ""} onChange={e => setForm({...form, receptionHours: e.target.value})} className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-indigo-500 mt-1" />
                  ) : (
                    <p className="text-lg font-bold text-gray-900">{profile.receptionHours || "—"}</p>
                  )}
                </div>
              </div>
              ...
              {editing && (
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={() => { setEditing(false); setForm(profile); }} className="px-6 py-3 bg-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-300 transition-all">Отмена</button>
                </div>
              )}
            </div>
          </div>
