          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {!showForgot ? (
              ...
              <div>
                ...
                <p className="mt-1.5">
                  <button type="button" onClick={() => { setShowForgot(true); setError(null); }} className="text-xs text-indigo-600 hover:text-indigo-800 font-medium underline underline-offset-2 bg-transparent border-none p-0 cursor-pointer">
                    Забыли пароль?
                  </button>
                </p>
              </div>
              ...
              <div className="space-y-5">
                {forgotSent ? (
                  <div className="text-center py-4">
                    ...
                    <p className="text-gray-800 font-bold mb-2">Проверьте почту</p>
                    <p className="text-sm text-gray-600 mb-4">Если аккаунт существует, мы отправили ссылку для сброса пароля.</p>
                    <button type="button" onClick={() => { setShowForgot(false); setForgotSent(false); setForgotEmail(""); }} className="text-indigo-600 hover:text-indigo-800 font-medium underline text-sm">
                      Вернуться ко входу
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Email для восстановления</label>
                      <input type="email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} placeholder="your@email.com" className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl text-gray-800 focus:outline-none focus:border-indigo-500 transition-all" />
                    </div>
                    <button type="button" onClick={handleForgotSubmit} disabled={forgotSending || !forgotEmail} className="w-full py-3 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-semibold rounded-xl transition-all disabled:opacity-50">
                      {forgotSending ? "Отправка..." : "Отправить ссылку"}
                    </button>
                    <button type="button" onClick={() => { setShowForgot(false); setError(null); }} className="w-full text-center text-sm text-indigo-600 hover:text-indigo-800 font-medium underline underline-offset-2">
                      Вернуться ко входу
                    </button>
                  </>
                )}
